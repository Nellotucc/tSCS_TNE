...
% this is the 
burst_single_phase_pulse_duration = uint32( 350); % us. Remember this is half of the duration of a bi-phasic pulse
...
burst_interpulse_duration = uint32( 300 ); % us. This is ideally set to 0 because there is no time between two bi-phasic pulses in a burster
...
burster_pulse_amplitude = 30.0 ; % mA


%% Try so send a few bursters

% activate the 0th channel without output - HV active
SetSingleChanState(s, 0, 1, 1, 0);
...
%%
for i = 1: 100
    SetSingleChanState(s, 0, 1, 1, 1); % Output and HV On

    pause(0.05);
    SetSingleChanState(s, 0, 1, 1, 0); % Output OFF
    pause(0.05);
    SetSingleChanState(s, 0, 1, 0, 0);
    pause(10);
